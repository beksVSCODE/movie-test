...
│   └── movie-filters/     # Фильтрация фильмов
...
- ✅ **Фильтрация фильмов** по жанрам, годам и рейтингу
- ✅ Пагинация результатов (до 500 страниц)
- ✅ Детальная информация о фильме
- ✅ Адаптивный дизайн

### Фильтры
- 🎭 **По жанру** - динамическая загрузка всех доступных жанров из TMDB
- 📅 **По году выпуска** - от 1900 до текущего года
- ⭐ **По рейтингу** - минимальный рейтинг от 5+ до 9+
- 🔄 **Комбинирование фильтров** - все фильтры работают вместе
- 🎯 **Работа с поиском** - фильтры применяются как к популярным фильмам, так и к результатам поиска
...
- Плавные анимации и transitions с градиентами
- Автоскролл вверх при смене страницы
- Кнопка "Назад" на детальной странице
- Placeholder для отсутствующих изображений
- Современный дизайн с эффектами подсветки и анимациями
- Градиентные фоны и кнопки с hover-эффектами
...
features/
├── search-movies/
│   ├── SearchBar.jsx           # Поиск с debounce
│   ├── SearchBar.styles.js     # Стили с градиентами
│   └── index.js
└── movie-filters/
    ├── MovieFilters.jsx        # Компонент фильтров
    ├── MovieFilters.styles.js  # Анимированные стили
    └── index.js
```
```
pages/
...
### Endpoints
- `GET /discover/movie` - Популярные фильмы с фильтрами
- `GET /search/movie` - Поиск фильмов с фильтрами
- `GET /movie/{id}` - Детали фильма
- `GET /genre/movie/list` - Список всех жанров

### Параметры фильтрации
- `with_genres` - ID жанра
- `primary_release_year` - Год выпуска
- `vote_average.gte` - Минимальный рейтинг
- `sort_by` - Сортировка (по умолчанию: popularity.desc)
...
- **Цветовая схема**: Темная тема Netflix-стайл с современными градиентами
- **Primary Color**: #e50914 (красный)
- **Background**: Анимированный градиентный фон
- **Типографика**: System fonts с градиентными заголовками
- **Анимации**: 
  - Плавные transitions (0.2s - 0.5s)
  - Keyframe анимации (fade-in, pulse, shine, gradient shift)
  - Hover эффекты с трансформациями
  - Градиентные подсветки и тени
- **UI Эффекты**:
  - Box-shadows с цветными отблесками
  - Backdrop-filter для эффекта размытия
  - Градиентные кнопки с анимацией
  - Плавающие элементы (floating animation)
  - Shimmer эффекты на hover
...
- [x] ~~Добавить фильтры по жанрам~~ ✅ Реализовано
- [x] ~~Добавить фильтры по годам и рейтингу~~ ✅ Реализовано
- [ ] Сохранение избранных фильмов (localStorage)
- [ ] Добавить сортировку (по дате, рейтингу, названию)
- [ ] Server-Side Rendering (Next.js миграция)
- [ ] Progressive Web App (PWA)
- [ ] Темная/светлая тема с переключателем
- [ ] Анимированные скелетоны для загрузки
...
**Основные фичи**: Поиск • Фильтрация • Пагинация • Адаптивный дизайн • Современные анимации
